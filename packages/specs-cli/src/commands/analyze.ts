...
import { getRedirectSummaryChain } from '../lib/redirects.js';
...
function executionSummaryFields(data: AnalysisResponse) {
  return {
    ...
    ...(getRedirectSummaryChain(data) ? { redirect_chain: getRedirectSummaryChain(data) } : {}),
  };
}

function executionSummarySuffix(data: AnalysisResponse): string {
  const redirectChain = getRedirectSummaryChain(data);
  if (!data.execution) {
    return redirectChain ? ` redirect_chain=${redirectChain}` : '';
  }

  return [
    data.execution.mode ? ` mode=${data.execution.mode}` : '',
    data.execution.engine ? ` engine=${data.execution.engine}` : '',
    data.execution.degraded !== undefined ? ` degraded=${String(data.execution.degraded)}` : '',
    data.execution.enrichmentStatus ? ` enrichment=${data.execution.enrichmentStatus}` : '',
    redirectChain ? ` redirect_chain=${redirectChain}` : '',
  ].join('');
}
...
export async function analyzeCommand(domain: string, options: AnalyzeOptions) {
  ...
  if (effectiveOptions.enrich && effectiveOptions.mode === 'cloud') {
    if (effectiveOptions.summaryJson) {
      console.log(
        JSON.stringify({
          ...
          drift_removed: 0,
          exit: 2,
          error: 'invalid_mode_enrich_combination',
        })
      );
      process.exit(2);
    }
    ...
    console.error('Cannot combine --enrich with --mode cloud. Use default local mode with --enrich.');
    process.exit(2);
  }
  ...
  if (effectiveOptions.summaryJson) {
    ...
    let data: unknown;
    try {
      data = await resolveAnalysis(normalizedDomain, {
        mode: effectiveOptions.mode,
        enrich: effectiveOptions.enrich,
      });
    } catch (error) {
  ...
  if (effectiveOptions.summary) {
    ...
    try {
      const data = await resolveAnalysis(normalizedDomain, {
        mode: effectiveOptions.mode,
        enrich: effectiveOptions.enrich,
      });
      ...
      if (effectiveOptions.diff) {
        ...
        console.log(
          `SUMMARY ${data.domain} status=${status} tech=${tech} seo=${seo} perf=${perf} hosting=${hosting} drift_changed=${changed} drift_added=${added} drift_removed=${removed} exit=${exit}${executionSummarySuffix(data)}${extra}`
        );
      ...
      console.log(
        `SUMMARY ${data.domain} status=${status} tech=${tech} seo=${seo} perf=${perf} hosting=${hosting}${executionSummarySuffix(data)}`
      );
      return;
    } catch (error) {
  ...
  try {
    ...
    if (data.execution?.enrichmentStatus === 'pending') {
      console.log();
      console.log(chalk.yellow('Optional SiteSpecs enrichment is still processing. The local analysis above is complete.'));
    }

    if (data.execution?.enrichmentStatus === 'failed') {
      console.log();
      console.log(chalk.yellow('Local analysis succeeded, but optional SiteSpecs enrichment was unavailable.'));
    }
