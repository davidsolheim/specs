...
import { resolveDefaultPackageSpecifier } from '../lib/package-release.js';
...
export async function ghaCommand(
  ...
  if (options.enrich && options.mode === "cloud") {
    console.error("GHA_ENRICH_MODE_CONFLICT");
    process.exit(2);
    return;
  }
  ...
  const pkg = options.version
    ...
    : resolveDefaultPackageSpecifier(options.cliVersion ?? '0.0.0');
  const modeArg = options.mode === "cloud" ? " --mode cloud" : "";
  const enrichArg = options.enrich ? " --enrich" : "";
