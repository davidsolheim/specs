...
describe('gha command', () => {
  const prereleaseCliVersion = '0.1.1-next.1775674433175';

  async function readFixture(name: string): Promise<string> {
    return await readFile(new URL(`./fixtures/gha/${name}`, import.meta.url), 'utf8');
  }

  function runGha(domain: string, options: Record<string, unknown> = {}) {
    return ghaCommand(domain, { cliVersion: prereleaseCliVersion, ...options } as any);
  }
  ...
  test('missing baseline: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(runGha('example.com', {})).rejects.toThrow('EXIT_2');
  ...
  test('missing domain: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(runGha('', { baseline: 'baseline.json' })).rejects.toThrow('EXIT_2');
    expect(errMock).toHaveBeenCalledTimes(1);
  ...
  test('success: prints exactly two YAML-ish lines', async () => {
    ...
    await runGha('example.com', { baseline: 'baseline.json' });
    ...
    expect(String((logMock as any).mock.calls[1][0])).toBe(
      '  run: npx -y @sitespecs/specs@next ci example.com --baseline baseline.json --fail-on-diff',
    );
  });

  test('success + --enrich: includes the enrichment flag in the generated snippet', async () => {
    const logMock = mock(() => {});
    console.log = logMock as typeof console.log;
    console.error = mock(() => {}) as typeof console.error;

    await runGha('example.com', { baseline: 'baseline.json', enrich: true } as any);

    expect(logMock).toHaveBeenCalledTimes(2);
    expect(String((logMock as any).mock.calls[1][0])).toBe(
      '  run: npx -y @sitespecs/specs@next ci example.com --baseline baseline.json --fail-on-diff --enrich',
    );
  });

  test('stable build defaults the generated snippet to @latest', async () => {
    const logMock = mock(() => {});
    console.log = logMock as typeof console.log;
    console.error = mock(() => {}) as typeof console.error;

    await runGha('example.com', { baseline: 'baseline.json', cliVersion: '0.1.0' } as any);
  ...
  test('workflow: prints full workflow YAML with trailing newline', async () => {
    ...
    await runGha('example.com', { baseline: 'baseline.json', workflow: true } as any);
  ...
  test('--fetch-depth without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', fetchDepth: 2 } as any),
  ...
  test('--artifact without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', artifact: 'specs-analysis.json' } as any),
    ).rejects.toThrow('EXIT_2');
    expect(errMock).toHaveBeenCalledTimes(1);
  ...
  test('--artifact-retention-days without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', artifactRetentionDays: 7 } as any),
    ).rejects.toThrow('EXIT_2');
    expect(errMock).toHaveBeenCalledTimes(1);
  ...
  test('--artifact-retention-days without --artifact: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', workflow: true, artifactRetentionDays: 7 } as any),
    ).rejects.toThrow('EXIT_2');
    expect(errMock).toHaveBeenCalledTimes(1);
  ...
  test('workflow + invalid --fetch-depth: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', workflow: true, fetchDepth: -1 } as any),
    ).rejects.toThrow('EXIT_2');

    await expect(
      runGha('example.com', { baseline: 'baseline.json', workflow: true, fetchDepth: 'nope' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('--artifact invalid: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', workflow: true, artifact: '' } as any),
    ).rejects.toThrow('EXIT_2');
    expect(errMock).toHaveBeenCalledTimes(1);
  ...
  test('--working-directory without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', workingDirectory: 'specs' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('--permissions without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', permissions: 'minimal' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('workflow + --permissions invalid: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', workflow: true, permissions: 'write-all' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('--runs-on without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', runsOn: 'foo' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('--concurrency without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', concurrency: 'specs-ci-example' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('--node-version without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', nodeVersion: '20' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('workflow + --concurrency blank: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', workflow: true, concurrency: '   ' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('workflow + --node-version blank: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', workflow: true, nodeVersion: '   ' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('workflow + --runs-on blank: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', workflow: true, runsOn: '   ' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('--timeout-minutes without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', timeoutMinutes: 10 } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('workflow + --timeout-minutes invalid: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', workflow: true, timeoutMinutes: 0 } as any),
    ).rejects.toThrow('EXIT_2');

    await expect(
      runGha('example.com', { baseline: 'baseline.json', workflow: true, timeoutMinutes: 'abc' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('--pull-request without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', pullRequest: true } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('--schedule without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(runGha('example.com', { baseline: 'baseline.json', schedule: '0 3 * * *' } as any)).rejects.toThrow(
      'EXIT_2',
    );
  ...
  test('--push without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(runGha('example.com', { baseline: 'baseline.json', push: true } as any)).rejects.toThrow('EXIT_2');
  ...
  test('--branch without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', branch: 'main' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('--name without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', name: 'My Specs CI' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('manual without workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', manual: true } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('workflow + manual: prints workflow.yml fixture', async () => {
    ...
    await runGha('example.com', { baseline: 'baseline.json', workflow: true, manual: true } as any);
  ...
  test('workflow + push: prints full workflow YAML with push trigger', async () => {
    ...
    await runGha('example.com', { baseline: 'baseline.json', workflow: true, push: true } as any);
  ...
  test('workflow + push + branch=master: prints workflow-push-branch-master.yml fixture', async () => {
    ...
    await runGha('example.com', { baseline: 'baseline.json', workflow: true, push: true, branch: 'master' } as any);
  ...
  test('workflow + branch (no push/pull-request): prints normal workflow.yml fixture', async () => {
    ...
    await runGha('example.com', { baseline: 'baseline.json', workflow: true, branch: 'release' } as any);
  ...
  test('write without workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(runGha('example.com', { baseline: 'baseline.json', write: 'workflow.yml' } as any)).rejects.toThrow(
      'EXIT_2',
    );
    expect(logMock).toHaveBeenCalledTimes(0);
    expect(errMock).toHaveBeenCalledTimes(1);
  ...
  test('workflow + write: writes YAML to disk and prints exactly one WORKFLOW_SAVED line', async () => {
    ...
    await runGha('example.com', { baseline: 'baseline.json', workflow: true, write: outPath } as any);
  ...
  test('workflow + write existing file (no --force): exits 2 with deterministic stderr and does not overwrite', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', workflow: true, write: outPath } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('workflow + --force without --write: exits 2 with deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', workflow: true, force: true } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('pinned version snippet: uses @<version> in npx', async () => {
    ...
    await runGha('example.com', { baseline: 'baseline.json', version: '0.1.0' } as any);
  ...
  test('--job without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', job: 'specs_ci' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('workflow + invalid --job id: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', workflow: true, job: '-bad' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('--job-name without --workflow: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', jobName: 'My Specs CI' } as any),
    ).rejects.toThrow('EXIT_2');
  ...
  test('workflow + invalid --job-name: exits 2 and prints deterministic stderr', async () => {
    ...
    await expect(
      runGha('example.com', { baseline: 'baseline.json', workflow: true, jobName: '' } as any),
    ).rejects.toThrow('EXIT_2');

    await expect(
      runGha('example.com', { baseline: 'baseline.json', workflow: true, jobName: 'a'.repeat(65) } as any),
    ).rejects.toThrow('EXIT_2');
