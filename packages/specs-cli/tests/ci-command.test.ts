...
import { ciCommand as runCiCommand } from '../src/commands/ci';

function ciCommand(domain: string, options: Record<string, unknown> = {}) {
  return runCiCommand(domain, { mode: 'cloud', ...options });
}
...
describe('ci command deterministic fixtures', () => {
  ...
  test('rate-limited API failure exits 1 with rate_limited marker', async () => {
    ...
    await expect(ciCommand('example.com', { baseline: baselinePath, mode: 'cloud' })).rejects.toThrow('EXIT_1');
  ...
  test('drift path + --fail-on-diff: baseline differs -> ok=false exit=1 and single JSON stdout line', async () => {
    const baseline = createAnalysis({ framework: 'Next.js' });
    const analysis = createAnalysis({ framework: 'Astro' });
