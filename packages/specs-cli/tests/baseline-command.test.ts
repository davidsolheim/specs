...
import { baselineCommand as runBaselineCommand } from '../src/commands/baseline';
...
function baselineCommand(domain: string, options: Record<string, unknown> = {}) {
  return runBaselineCommand(domain, { mode: 'cloud', ...options });
}
...
describe('baseline command', () => {
  ...
  test('happy path: writes baseline file and prints exactly one BASELINE_SAVED line', async () => {
    ...
    await baselineCommand('https://www.Example.com/', { out: outPath, mode: 'cloud' });
  ...
  test('--stdout: prints exactly one JSON line and no other logs', async () => {
    ...
    await baselineCommand('https://www.Example.com/', { stdout: true, mode: 'cloud' });
    ...
    expect(logMock).toHaveBeenCalledTimes(1);
    const line = String((logMock as any).mock.calls[0][0]);
    expect(JSON.parse(line)).toMatchObject(withCloudExecution(payload));
  });
  ...
  test('out path exists (--force): overwrites and prints BASELINE_SAVED', async () => {
    ...
    await baselineCommand('example.com', { out: outPath, force: true, mode: 'cloud' });
  ...
  test('api failure: repeated 528 exits 1 with upstream_unavailable marker', async () => {
    global.fetch = mock(async () =>
